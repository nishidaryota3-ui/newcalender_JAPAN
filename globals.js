// globals.js (全体のデータと状態の管理) - オフライン天文学計算（出没）対応版
...
// ▼ 天文学計算用エンジン (SunCalc 軽量版) ▼
// 日の出・日の入り、月の出・月の入りをオフラインで瞬時に計算するための関数群です。
...
function getTimes(date, lat, lng, height) {
    var lw = rad * -lng, phi = rad * lat, d = toDays(date), n = Math.round(d - lw / (2 * PI)), h0 = -0.0053 - 2.076 * Math.sqrt(height || 0) / 60;
    var c = sunCoords(d + n), H = Math.acos((Math.sin(h0) - Math.sin(phi) * Math.sin(c.dec)) / (Math.cos(phi) * Math.cos(c.dec)));
    var Jnoon = 2451545 + n + 0.0009 - lw / (2 * PI), Jset = Jnoon + H / (2 * PI), Jrise = Jnoon - H / (2 * PI);
...
function getMoonTimes(date, lat, lng) {
    ...
    for (var i = 1; i <= 24; i += 2) {
        ...
        if (roots === 1) {
            ...
            rise = i + (ye < 0 ? x2 : x1); set = i + (ye < 0 ? x1 : x2);
        }
